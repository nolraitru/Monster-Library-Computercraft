...
-- Script
if #tArgs == 0 then
	MonLib.Error("Use [StripMine <length> <branches>].")
	return
end
...
if turtle.getItemCount(storage) < 6 then
	MonLib.Error("Atleast 6 chests in slot 14.")
	return
end
 
if turtle.getItemCount(fuel) < 32 then
	MonLib.Error("Atleast 32 fuel in slot 16.")
	return
end
 
if turtle.getItemCount(cobble) < 64 then
	MonLib.Error("A stack of cobble in slot 13.")
	return
end
